...
// 级别色用主题 token(info 用 foreground 而非 muted,保证日志正文可读;warn 用 warning,非荧光)。
...
export function ModelLogPanel({ m }: { m: ModelInfo }) {
  ...
  return (
    <div className="flex h-full flex-col overflow-hidden rounded-lg border border-border bg-card">
      ...
      <div className="flex items-center gap-2 border-b border-border bg-muted/30 px-3.5 py-1.5 text-[10.5px]">
        {LEVELS.map((lv) => (
