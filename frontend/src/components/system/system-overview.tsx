...
// (更新范围 稳定版|最新提交、当前版本(版本(提交短号))、最新(落后 N)、检查更新/更新)合并在第一行。
//
...
const MODES: readonly { key: UpdateTarget; label: string }[] = [
  { key: "tag", label: "稳定版" },
  { key: "commit", label: "最新提交" },
];
const modeLabel = (mode: UpdateTarget) => (mode === "tag" ? "稳定版" : "最新提交");
const otherModeLabel = (mode: UpdateTarget) => (mode === "tag" ? "最新提交" : "稳定版");
...
export function SystemOverview() {
  ...
  const onUpdate = async () => {
    ...
    const ok = await confirm({
      title: `更新到${modeLabel(mode)} ${label}?`,
  ...
  if (st?.checking || (isFetching && !st)) {
    ...
    if (!st.ok) {
      ...
      note = "本地与远端历史分叉,无法自动更新,需手动处理";
      noteClass = "text-warning";
    } else if (isAvailable(st, mode)) {
      note = `发现新${mode === "tag" ? "版本" : "提交"} — ${latest}`;
      noteClass = "text-success";
    } else if (mode === "tag" && !st.tag) {
      note = "项目未发布过稳定版,可切换到「最新提交」模式更新";
    } else if (otherAvailable(st, mode)) {
      note = `已是最新${mode === "tag" ? "稳定版" : "提交"},「${otherModeLabel(mode)}」模式还有更新可拉取`;
    } else {
      note = "已是最新,无需更新";
      noteClass = "text-success";
    }
  } else if (isError) {
    note = "获取更新信息失败,可点击「检查更新」重试";
    noteClass = "text-destructive";
  } else {
    note = "点击「检查更新」手动检查最新状态";
  }
    ...
    <div className="mb-4">
      {!unsupported && (
        <div className="mb-2 flex flex-wrap items-center gap-3">
          ...
          <div className="flex items-center gap-2">
            <span className="text-xs text-muted-foreground">更新范围</span>
        ...
        {!unsupported && (
          <>
            ...
            <InfoTile
              ...
              value={`${latest}${behind > 0 ? ` (落后 ${behind} 个提交)` : ""}`}
      ...
      {!unsupported && (
        <div className="mt-2 flex flex-wrap items-center gap-x-4 gap-y-1">
          ...
          {st?.dirty && <p className="text-xs text-warning">有本地未提交改动 — 仅当与更新内容冲突时才会被拒绝,否则自动保留</p>}
        </div>
      )}
