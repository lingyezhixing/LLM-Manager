...
### v2.4.0 - 2026-01-05 (Current)
**设备监控增强**
- **AMD 780M 核显支持**：新增对 AMD Radeon 780M 核显的完整监控能力
- **智能温度读取**：CPU 温度监控实现降级策略，优先 CPU 传感器，失败时自动降级到核显温度
- **监控库集成**：集成 LibreHardwareMonitor 库，提供更精准的硬件状态读取
- **代码质量优化**：清理 API 服务器冗余注释，归一化日志输出格式

### v2.3.0 - 2026-01-04
**启动控制与性能优化**
- **打断启动功能**：支持中止正在启动中的模型，避免长时间等待
- **并行关闭机制**：实现模型并行卸载，程序关闭速度提升 80%+
- **智能资源释放**：优化集中卸载策略，自动评估并释放空闲模型资源
- **代码重构优化**：减少重复逻辑，提升代码可维护性；统一版本号管理，集中配置于 main.py
- **进程管理增强**：减少子进程残留，确保进程生命周期可控
- **日志系统优化**：统一日志输出格式，提升可读性
