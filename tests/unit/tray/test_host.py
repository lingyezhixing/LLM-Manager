...
from llm_manager.tray import host
...
def test_is_tray_available_false_when_pystray_missing(monkeypatch):
    monkeypatch.setattr(host, "_PYSTRAY_AVAILABLE", False)
    assert host.is_tray_available() is False


def test_is_tray_available_true_when_present(monkeypatch):
    monkeypatch.setattr(host, "_PYSTRAY_AVAILABLE", True)
    assert host.is_tray_available() is True


def test_is_headless_display_posix_without_display(monkeypatch):
    monkeypatch.setattr(host.os, "name", "posix")
    monkeypatch.delenv("DISPLAY", raising=False)
    assert host._is_headless_display() is True
    monkeypatch.setenv("DISPLAY", ":0")
    assert host._is_headless_display() is False
...
def test_apply_claude_unknown_preset_noop(monkeypatch, tmp_path):
    tray = _make_tray(settings_path=tmp_path / "s.json")
    called = []
    monkeypatch.setattr(host.claude, "apply_preset", lambda *a: called.append(a))
    tray.apply_claude("Nonexistent")
    assert called == []
...
async def test_unload_all_marshals_lifecycle_unload_all():
    life = _FakeLife()
    tray = _make_tray(lifecycle=life)
    captured = []
...
async def test_restart_auto_start_unloads_then_autostarts(monkeypatch):
    ...
    monkeypatch.setattr(host.background, "auto_start", fake_auto_start)
    tray.restart_auto_start()
    assert len(captured) == 1
    await captured[0]
    assert life.unload_called is True
...
def test_exit_app_sets_server_should_exit():
    server = _FakeServer()
    loop = _FakeLoop()
    tray = _make_tray(loop=loop, server=server)
    tray.exit_app()
