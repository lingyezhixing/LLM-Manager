...
def test_inject_include_usage_preserves_existing_stream_options():
    out = proxy._inject_include_usage({"model": "m1", "stream": True, "stream_options": {"foo": 1}}, "v1/chat/completions")
    assert out["stream_options"]["include_usage"] is True
...
def _make_request(method, path, json_body, content_type="application/json"):
    ...
    scope = {
        "type": "http", "method": method, "path": path.split("/"),
...
async def test_forward_non_stream_records_usage_and_ends_request():
    ...
    def handler(req):
        return httpx.Response(200, json={"id": "x", "usage": {"prompt_tokens": 4, "completion_tokens": 6, "total_tokens": 10}},
                              headers={"content-type": "application/json"})
    client = httpx.AsyncClient(base_url="http://127.0.0.1:8000", transport=httpx.MockTransport(handler))
    db = open_db(Path(":memory:"))
...
async def test_forward_ensure_running_failed_returns_503():
    state._reset()
    client = httpx.AsyncClient(base_url="http://127.0.0.1:8000", transport=httpx.MockTransport(lambda r: httpx.Response(200)))
    db = open_db(Path(":memory:"))
    req = _make_request("POST", "v1/chat/completions", {"model": "m1"})
    with pytest.raises(HTTPException) as ei:
        await proxy.forward(req, "v1/chat/completions", FakeLifecycle(ModelStatus.FAILED), _cfg(), db, {8000: client})
