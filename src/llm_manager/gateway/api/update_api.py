...
状态:fetch 后以 git 标签版本身份对比本地/远端(不动工作树);
应用:fetch + ff-only 合并(严格模式,脏/分叉 → 409)→ 成功后走
...
def register_update_routes(api: APIRouter) -> None:
    ...
    async def update_apply(request: Request) -> dict:
        """拉取最新代码并重启:fetch + ff-only 合并 → trigger_restart(202 先冲刷,
        worker 优雅收口后以 81 退出,parent 拉新 worker 加载新代码)。
        工作树脏 / 历史分叉 / 网络失败 → 409,不动本地任何东西。
        async def 使 trigger_restart 的 asyncio.create_task 落在事件循环;阻塞的 git
        拉取经 to_thread 外包线程池。"""
        try:
            new_sha = await asyncio.to_thread(apply_update)
        except UpdateError as e:
            raise HTTPException(status_code=409, detail=str(e)) from e
        trigger_restart(request)
        return {"updated": True, "sha": new_sha}
