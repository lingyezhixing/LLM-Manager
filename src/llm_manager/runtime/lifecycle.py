...
class Lifecycle:
    def __init__(
        ...
        *,
        ...
        self._log_session_ids: dict[
            str, int
        ] = {}  # alias → 进行中模型日志会话 id(多模型并发,按 alias 独立追踪)
        self._runtime_seg_ids: dict[
            str, int
        ] = {}  # alias → 进行中计费运行段 id(record_runtime_end 按关,不再靠 end_time IS NULL 定位)
        ...
        try:
            ...
            if not probe.ok:
                ...
                if ev.is_set():
                    return (
                        ModelStatus.STOPPED
                    )  # kill_tree awaited; stop may have come — don't overwrite STOPPED
                self._log_end(
                    alias
                )  # probe 失败不会走 on_exit / stop(FAILED 早退)→ 必须在此收口,防泄漏 running 会话
